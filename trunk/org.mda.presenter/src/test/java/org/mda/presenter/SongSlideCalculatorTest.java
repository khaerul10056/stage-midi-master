...
public class SongSlideCalculatorTest {
	private static final Log LOGGER  = LogFactory.getLogger(SongSlideCalculatorTest.class);
	  ...
	  public void createChordRelatedPart () throws Exception {
	    Song song = SongCreator.create().part(SongPartType.INTRO).line().chordAndText("D", " ").get();
	  ...
	  public void createLines () {
		SongCreator songcreator = SongCreator.create().part(SongPartType.INTRO).line().chordAndText("D", "  "); 
		Song song = songcreator.part(SongPartType.VERS).line().text("This is first line and it is so long that it should be autowrapped if configured, but now we try to wrap it NOT").get();
	    ...
	    assertEquals ("This is first line and it is so long that it should be autowrapped if configured, but now we try to wrap it NOT", calculatePart.getTextline(0));
	  ...
	  public void pagePerPart () {
	    ...
	    SongCreator creator = SongCreator.create().part(SongPartType.REFRAIN);
	    ...
	    assertTrue (thirdLineItem.getY() > 10);
